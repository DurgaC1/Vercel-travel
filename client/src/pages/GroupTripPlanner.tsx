...
//   "Hindu temple", "Heritage museum", "Garden", "Park", "Museum", "Historical landmark",
//   "Historical place museum", "Public beach", "Catholic cathedral", "Hill station",
//   "Scenic spot", "Nature preserve", "Vista point", "Archaeological site", "Mosque",
//   "Religious destination", "Fortress", "History museum", "Catholic church", "Shrine",
//   "National park", "Jain temple", "Archaeological museum", "Ecological park",
//   "Heritage building", "Lake", "Observatory", "Church", "Art museum", "Monastery",
//   "Buddhist temple", "Hiking area", "Anglican church", "National forest", "Monument",
//   "Gurudwara", "Castle", "Wax museum", "Wildlife refuge", "Wildlife and safari park",
//   "Wildlife park", "Beach pavillion", "Beach",
// ];
...
//   const [expenseForm, setExpenseForm] = useState({ description: "", amount: "", paidBy: auth.currentUser?.displayName || "You" });
...
//         setTripPlan((prev) => ({
//           ...prev,
//           members: [{ id: user.uid, name: user.displayName || "You", role: "Organizer", status: "Confirmed" }],
//         }));
//         setExpenseForm((prev) => ({ ...prev, paidBy: user.displayName || "You" }));
//         setActivityForm((prev) => ({ ...prev, proposedBy: user.displayName || "You" }));
//         // Load existing trip if tripId is in URL
...
//                 activities: 0,
//                 hotels: data.itinerary?.days?.find((d: any) => d.day === i + 1)?.hotels || [],
//               });
//             }
...
//           }
//         }
//         // Subscribe to chat messages, activities, and expenses
//         if (tripId) {
//           const messagesQuery = query(collection(db, `trips/${tripId}/messages`), orderBy("timestamp", "asc"));
//           const unsubscribeMessages = onSnapshot(messagesQuery, (snapshot) => {
//             const messages = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as { id: string; userName: string; message: string; timestamp: string }));
//             setChatMessages(messages);
//           });
//           const activitiesQuery = query(collection(db, `trips/${tripId}/activities`));
...
//       const data = await response.json();
//       return data.results?.[0]?.urls?.regular || "https://images.pexels.com/photos/1268871/pexels-photo-1268871.jpeg";
//     } catch (err) {
...
//           HotelImage: hotel.HotelImage || "https://images.pexels.com/photos/271624/pexels-photo-271624.jpeg",
//           reactions: [],
//         }));
...
//       const tripResponse = await fetch("http://localhost:3001/api/trips", {
//         method: "POST",
//         headers: {
...
//       const itinerary = USE_MOCK_DATA ? mockItinerary : await fetchDatasetData(tripPlan.destination, tripPlan.numberOfPersons, tripPlan.categories);
...
//           (itinerary.days[i]?.activities || []).map(async (act: any, idx: number) => {
...
//               proposedBy: auth.currentUser?.displayName || "You",
//             };
//             const activityResponse = await fetch(`http://localhost:3001/api/trips/${tripId}/activities`, {
//               method: "POST",
//               headers: {
//                 "Content-Type": "application/json",
//                 Authorization: `Bearer ${token}`,
...
//     const token = await auth.currentUser?.getIdToken(true);
//     try {
//       const response = await fetch(`http://localhost:3001/api/trips/${tripPlan.tripId}/invite`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
//           Authorization: `Bearer ${token}`,
...
//         throw new Error(`Failed to invite friend: ${errorText}`);
//       }
...
//     if (!activity.id || !tripPlan.tripId) return;
//     const user = auth.currentUser;
//     if (!user) return;
//     const token = await user.getIdToken(true);
//     const existingReaction = activity.reactions.find(r => r.user._id === user.uid);
...
//     }
//     try {
//       await fetch(`http://localhost:3001/api/trips/${tripPlan.tripId}/activities/${activity.id}`, {
//         method: "PATCH",
//         headers: {
//           "Content-Type": "application/json",
//           Authorization: `Bearer ${token}`,
...
//         ),
//       }));
//       toast.success(`${type === "like" ? "Liked" : "Disliked"} ${activity.title}!`);
//     } catch (err: any) {
...
//       };
//       const response = await fetch(`http://localhost:3001/api/trips/${tripPlan.tripId}/activities`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
//           Authorization: `Bearer ${token}`,
...
//       }));
//       setTripDays(prev => prev.map(day =>
//         day.day === activityForm.day ? { ...day, activities: day.activities + 1 } : day
//       ));
//       setActivityForm({
//         day: 1,
//         time: "",
...
//     const token = await auth.currentUser?.getIdToken(true);
//     try {
//       const response = await fetch(`http://localhost:3001/api/trips/${tripPlan.tripId}/expenses`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
//           Authorization: `Bearer ${token}`,
...
//       setExpenses((prev) => [
//         ...prev,
//         { id: expenseId, description: expenseForm.description, amount, paidBy: expenseForm.paidBy },
//       ]);
...
//     }
//     const user = auth.currentUser;
//     if (user && tripPlan.tripId) {
//       const token = await user.getIdToken(true);
//       try {
//         const response = await fetch(`http://localhost:3001/api/trips/${tripPlan.tripId}/messages`, {
//           method: "POST",
//           headers: {
//             "Content-Type": "application/json",
//             Authorization: `Bearer ${token}`,
...
//       return;
//     }
//     const shareUrl = `${window.location.origin}/group-trip?tripId=${tripPlan.tripId}`;
...
//                   <span className="text-gray-800">{member.name} {member.role === "Organizer" ? "(You)" : `(${member.status})`}</span>
...
//                   <Card key={activity.id || index} className="p-4 mb-4 bg-white shadow-md">
...
//                             >
//                               Like ({activity.reactions.filter(r => r.type === "like").length})
//                             </Button>
//                           </motion.div>
//                           <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
//                             <Button
...
//                             >
//                               Dislike ({activity.reactions.filter(r => r.type === "dislike").length})
//                             </Button>
//                           </motion.div>
//                         </div>
//                         <div className="flex gap-2 mt-2">
...
//                         <p className="text-sm font-medium text-gray-800 w-16 text-right">${expense.amount.toFixed(2)}</p>
//                       </div>
//                     </Card>
//                   ))
//                 )}
...
//                         {msg.userName} <span className="text-xs text-gray-500">{new Date(msg.timestamp).toLocaleTimeString()}</span>
//                       </p>
...
//                           }));
//                           setTripDays(prev => prev.map(day =>
//                             day.day === suggestion.day ? { ...day, activities: day.activities + 1 } : day
//                           ));
...
const GroupTripPlanner = () => {
  ...
  const queryTripId = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("tripId") : null;
  ...
  useEffect(() => {
    ...
    const messagesQuery = query(collection(db, `trips/${routeTripId}/messages`), orderBy("timestamp", "asc"));
    const unsubscribeMessages = onSnapshot(
      ...
      (snapshot) => {
        const messages = snapshot.docs.map((doc) => {
          ...
          if (d.timestamp) {
            if (typeof (d.timestamp as any).toDate === "function") tsISO = (d.timestamp as any).toDate().toISOString();
            else if ((d.timestamp as any).seconds) tsISO = new Date((d.timestamp as any).seconds * 1000).toISOString();
            else if (typeof d.timestamp === "string") tsISO = new Date(d.timestamp).toISOString();
          }
          return { id: doc.id, userName: d.userName || "Unknown", message: d.message || "", timestamp: tsISO };
        });
        setChatMessages(messages);
        setTimeout(() => {
    ...
    const activitiesQuery = query(collection(db, `trips/${routeTripId}/activities`));
    const unsubscribeActivities = onSnapshot(
      ...
      (snapshot) => {
        ...
        setDayActivities(newDayActivities);
        setTripDays((prev) => prev.map((day) => ({ ...day, activities: newDayActivities[day.day]?.length || 0 })));
        setAiSuggestions(newDayActivities[1]?.slice(0, 3) || []);
      },
      (err) => {
    ...
    const expensesQuery = query(collection(db, `trips/${routeTripId}/expenses`));
    const unsubscribeExpenses = onSnapshot(
      expensesQuery,
      (snapshot) => {
        const expenses = snapshot.docs.map((doc) => ({ id: doc.id, ...(doc.data() as any) } as Expense));
        setExpenses(expenses);
      },
      (err) => {
  ...
  const fetchItinerary = async () => {
    ...
    try {
      ...
      const itinerary: ItineraryResponse = USE_MOCK_DATA
        ? mockItinerary
        : await fetchDatasetData(tripPlan.destination, tripPlan.numberOfPersons, tripPlan.categories);

      // 3) PATCH itinerary to trip doc so GET /trips/:id includes hotels immediately
      try {
        const patchRes = await fetch(`http://localhost:3002/api/trips/${tripId}`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
      ...
      for (let i = 0; i < duration; i++) {
        ...
        date.setDate(startDate.getDate() + i);
        const dayIndex = i;
        const itineraryDay = itinerary.days[dayIndex] || itinerary.days[0] || { activities: [], hotels: [] };
        ...
        const activitiesWithImages = await Promise.all(
          (itineraryDay.activities || []).map(async (act: any, idx: number) => {
            ...
            try {
              const activityResponse = await fetch(`http://localhost:3002/api/trips/${tripId}/activities`, {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
                  Authorization: `Bearer ${token}`,
  ...
  const handleInviteFriend = async (email: string) => {
    ...
    const token = await auth.currentUser?.getIdToken(true);
    try {
      const res = await fetch(`http://localhost:3002/api/trips/${activeTripId}/invite`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      // Try to parse JSON whether the status is ok or not
      let body: any = null;
      try {
        body = await res.json();
      } catch (parseErr) {
        // fallback if server returned non-JSON (rare)
        console.warn("Failed to parse invite response JSON", parseErr);
      }

      if (!res.ok) {
        // Prefer server-provided message, otherwise use status text
        const serverMessage = body?.message || (await res.text()).slice(0, 300) || res.statusText;
        throw new Error(serverMessage);
      }

      // At this point res.ok === true
      const returnedEmail = body?.email || email;
      const serverMessage = (body?.message || "").toLowerCase();

      if (serverMessage.includes("email not sent") || serverMessage.includes("email not sent - smtp")) {
        // Invite recorded but SMTP not configured on server
        toast.success(`Invite recorded for ${returnedEmail} (email not sent - SMTP not configured).`);
      } else {
        // Normal success path when email was sent
        toast.success(`Email invite sent to ${returnedEmail}.`);
      }

      // Optionally update UI: add a placeholder invited member or refresh invites
      // e.g. setTripPlan(prev => ({ ...prev, members: [...prev.members, { id: `invited-${Date.now()}`, name: returnedEmail.split('@')[0], role: 'Guest', status: 'Invited' }] }));
  ...
  const handleVote = async (day: number, activityIndex: number, type: "like" | "dislike") => {
    ...
    try {
      await fetch(`http://localhost:3002/api/trips/${activeTripId}/activities/${activity.id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleAddActivity = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingForDay.some((a) => (a.title || "").trim().toLowerCase() === newTitleNorm)) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try { errBody = await response.clone().json(); } catch (_) { /* not JSON */ }
        const errMsg = errBody?.message || (await response.text()).slice(0, 200) || response.statusText;
      ...
      setTripDays((prev) => prev.map((d) => (d.day === activityForm.day ? { ...d, activities: (d.activities || 0) + 1 } : d)));
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    const token = await auth.currentUser?.getIdToken(true);
    try {
      const response = await fetch(`http://localhost:3002/api/trips/${activeTripId}/expenses`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleAddChatMessage = async (message: string) => {
    ...
    if (user && activeTripId) {
      ...
      try {
        ...
        setChatMessages((prev) => [...prev, { id: optimisticId, userName: user.displayName || "You", message, timestamp: nowISO }]);
        const response = await fetch(`http://localhost:3002/api/trips/${activeTripId}/messages`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
    ...
    <div className="min-h-screen bg-gray-50">
                ...
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        className="border-gray-300 focus:ring-purple-500 focus:border-purple-500"
                      />
                      <datalist id="cities">{cities.map((city) => (<option key={city.id} value={city.name} />))}</datalist>
                      <p id="destination-help" className="text-sm text-gray-500">Enter a city, like 'Hyderabad' or 'Mumbai'.</p>
                    </div>
                    <div>
                      <Label htmlFor="numberOfPersons">Number of Persons</Label>
                  ...
                  <div>
                    ...
                    <Select value="" onValueChange={(value) => {
                      setTripPlan((prev) => {
                        const newCategories = prev.categories.includes(value) ? prev.categories.filter((c) => c !== value) : [...prev.categories, value];
                        return { ...prev, categories: newCategories };
                      });
                    }}>
                      <SelectTrigger className="bg-white border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500 text-gray-800 font-medium py-3 text-lg">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {tripPlan.categories.map((category) => (
                        <Badge key={category} variant="secondary" className="cursor-pointer bg-purple-100 text-purple-800" onClick={() => {
                          setTripPlan((prev) => ({ ...prev, categories: prev.categories.filter((c) => c !== category) }));
                        }}>{category} ✕</Badge>
                      ))}
                    </div>
                  </div>
                  <Button type="submit" className="w-full bg-purple-500 hover:bg-purple-600 text-white font-semibold py-3 rounded-full" aria-label="Create trip plan" disabled={isFetchingItinerary}>
                    {isFetchingItinerary ? (<span className="flex items-center"><motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-4 h-4 border-t-2 border-b-2 border-white mr-2" />Creating...</span>) : "Create Trip Plan"}
                  </Button>
                </form>
              </DialogContent>
            </Dialog>
      ...
      <div className="container mx-auto px-4 py-16">
        {error && error !== "This activity is already present" &&(
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-red-50 text-red-700 p-4 rounded-lg mb-8 shadow-sm max-w-2xl mx-auto text-center" role="alert">
            {error}
            <div className="mt-2 flex gap-2 justify-center">
              <Button variant="link" className="text-red-700 underline" onClick={fetchItinerary} aria-label="Retry creating itinerary">Retry</Button>
              <Button variant="link" className="text-red-700 underline" onClick={() => { setTripPlan({ ...tripPlan, destination: "Sample City" }); fetchItinerary(); }} aria-label="Use mock data">Use Sample Data</Button>
            </div>
          </motion.div>
        )}
        {isFetchingItinerary && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-purple-50 text-purple-700 p-4 rounded-lg mb-8 shadow-sm max-w-2xl mx-auto text-center">
            <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="inline-block w-6 h-6 border-t-2 border-b-2 border-purple-500 mr-2" />
            Generating your itinerary...
          </motion.div>
        )}
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="w-full md:w-80 bg-white rounded-lg shadow p-4">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Your Group Trip</h2>
            <p className="text-sm text-gray-600 mb-4">
              {tripDays.length > 0 ? `${tripPlan.name || "Untitled Trip"} • ${tripDays[0].date} - ${tripDays[tripDays.length - 1].date} • ${tripPlan.members.length} friends • ${tripPlan.numberOfPersons} persons` : "Start planning to see details"}
            </p>
            ...
            <div className="mb-4">
              ...
              {tripPlan.members.map((member) => (
                <div key={member.id} className="flex items-center justify-between mb-2">
                  <span className="text-gray-800">{member.name} {member.role === "Organizer" ? "(You)" : `(${member.status})`}</span>
                  {member.role !== "Organizer" && (<Button variant="ghost" size="sm" aria-label={`Remove ${member.name}`}>Remove</Button>)}
                </div>
              ))}
              <Input placeholder="Invite a friend (e.g., friend@email.com)" onKeyDown={(e) => {
                if (e.key === "Enter") { handleInviteFriend(e.currentTarget.value); e.currentTarget.value = ""; }
              }} className="mt-2 border-gray-300 focus:ring-purple-500 focus:border-purple-500" aria-label="Invite a friend by email" />
            </div>
            ...
            {tripDays.map((day) => {
              ...
              const activitiesCount = (dayActivities?.[day.day]?.length ?? day.activities ?? 0);

              return (
                <button
                  key={day.day}
                  onClick={() => setSelectedDay(day.day)}
                  className={`w-full p-3 rounded-lg text-left mb-2 ${selectedDay === day.day ? "bg-purple-100 text-purple-800" : "bg-gray-100"}`}
                  aria-label={`Select Day ${day.day} (${day.date})`}
                >
                  <div className="flex justify-between">
                    <span>Day {day.day} ({day.date})</span>
                    <Badge className="bg-purple-100 text-purple-800">{activitiesCount} activities</Badge>
                  </div>
                </button>
              );
            })}
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button variant="outline" className="w-full mt-4 border-purple-500 text-gray-800 hover:bg-purple-50 hover:text-purple-600 bg-white z-10" aria-label="Share trip" onClick={handleShareTrip}>
                <Share className="w-4 h-4 mr-2 text-gray-800" />
                Share Trip
              </Button>
            </motion.div>
          ...
          <div className="flex-1">
            <Tabs defaultValue="itinerary">
              <TabsList className="grid grid-cols-3 mb-4 bg-gray-100">
                <TabsTrigger value="itinerary" className="data-[state=active]:bg-purple-100 data-[state=active]:text-purple-800">Plan</TabsTrigger>
                <TabsTrigger value="expenses" className="data-[state=active]:bg-purple-100 data-[state=active]:text-purple-800">Expenses</TabsTrigger>
                <TabsTrigger value="chat" className="data-[state=active]:bg-purple-100 data-[state=active]:text-purple-800">Chat</TabsTrigger>
              </TabsList>

              <TabsContent value="itinerary">
                <h2 className="text-lg font-semibold text-gray-800 mb-4">Day {selectedDay} - {tripDays.find((d) => d.day === selectedDay)?.date || "Pick a day"}</h2>

                <Dialog open={isActivityFormOpen} onOpenChange={setIsActivityFormOpen}>
                  <DialogTrigger asChild>
                    <Button className="mb-4 bg-purple-500 text-white hover:bg-purple-600 rounded-full" aria-label="Add activity">
                      <Plus className="w-4 h-4 mr-2" />
                      Add Activity
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="sm:max-w-xl">
                    ...
                    <form onSubmit={handleAddActivity} className="space-y-4">
                      <div>
                        <Label htmlFor="activity-day">Day</Label>
                        <Select value={activityForm.day.toString()} onValueChange={(value) => setActivityForm({ ...activityForm, day: parseInt(value) })}>
                          <SelectTrigger>
                            <SelectValue placeholder="Select day" />
                          </SelectTrigger>
                          <SelectContent>
                            {tripDays.map((day) => (
                              <SelectItem key={day.day} value={day.day.toString()}>Day {day.day} ({day.date})</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </div>
                      <div>
                        <Label htmlFor="activity-time">Time</Label>
                        <Input id="activity-time" type="time" value={activityForm.time} onChange={(e) => setActivityForm({ ...activityForm, time: e.target.value })} className="border-gray-300 focus:ring-purple-500 focus:border-purple-500" />
                      </div>
                      <div>
                        <Label htmlFor="activity-title">Activity Name</Label>
                        <Input id="activity-title" value={activityForm.title} onChange={(e) => setActivityForm({ ...activityForm, title: e.target.value })} placeholder="e.g., Visit Taj Mahal" className="border-gray-300 focus:ring-purple-500 focus:border-purple-500" />
                      </div>
                      <div>
                        <Label htmlFor="activity-type">Category</Label>
                      ...
                      <div>
                        <Label htmlFor="activity-description">Description</Label>
                        <Input id="activity-description" value={activityForm.description} onChange={(e) => setActivityForm({ ...activityForm, description: e.target.value })} placeholder="e.g., Explore the historic monument" className="border-gray-300 focus:ring-purple-500 focus:border-purple-500" />
                      </div>
                      <Button type="submit" className="w-full bg-purple-500 hover:bg-purple-600 text-white font-semibold py-3 rounded-full" disabled={isFetchingItinerary}>Add Activity</Button>
                    </form>
                  </DialogContent>
                </Dialog>
                  ...
                  <div className="mb-4">
                    ...
                    {tripDays.find((d) => d.day === selectedDay)?.hotels.map((hotel, index) => (
                      <Card key={index} className="p-4 mb-4 bg-white shadow-md">
                        ...
                        <p className="text-sm text-gray-600">Attractions: {hotel.CleanedAttractions}</p>
                        {hotel.HotelWebsiteUrl !== "No website available" && (
                          <a href={hotel.HotelWebsiteUrl} target="_blank" rel="noopener noreferrer" className="text-purple-500 text-sm hover:underline">Visit Website</a>
                        )}
                        {hotel.HotelImage && (<img src={hotel.HotelImage} alt={hotel.HotelName} className="w-full h-32 object-cover rounded-md mt-2" />)}
                        <div className="flex gap-2 mt-2">
                          {hotel.reactions.map((r, i) => (<Badge key={i} variant="outline" className="flex items-center gap-1">{r.type === "like" ? <ThumbsUp className="w-3 h-3" /> : <ThumbsDown className="w-3 h-3" />}{r.user.name}</Badge>))}
                        </div>
                      </Card>
                    ))}
                  </div>
                  ...
                  <Card key={activity.id || index} className="p-4 mb-4 bg-white shadow-md">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex gap-2 mt-2">
                          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                            <Button variant="outline" size="sm" className="border-purple-500 text-purple-500 hover:bg-purple-50 hover:text-purple-600 bg-white z-10" onClick={() => handleVote(selectedDay, index, "like")} aria-label={`Like ${activity.title}`}>
                              Like ({activity.reactions.filter((r) => r.type === "like").length})
                            </Button>
                          </motion.div>
                          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                            <Button variant="outline" size="sm" className="border-purple-500 text-purple-500 hover:bg-purple-50 hover:text-purple-600 bg-white z-10" onClick={() => handleVote(selectedDay, index, "dislike")} aria-label={`Dislike ${activity.title}`}>
                              Dislike ({activity.reactions.filter((r) => r.type === "dislike").length})
                            </Button>
                          </motion.div>
                        </div>
                        <div className="flex gap-2 mt-2">
                          {activity.reactions.map((r, i) => (<Badge key={i} variant="outline" className="flex items-center gap-1">{r.type === "like" ? <ThumbsUp className="w-3 h-3" /> : <ThumbsDown className="w-3 h-3" />}{r.user.name}</Badge>))}
                        </div>
                      </div>
                    </div>
                  </Card>
              ...
              <TabsContent value="expenses">
                ...
                <Card className="p-4 mb-4 bg-white shadow-md">
                  <form onSubmit={handleAddExpense} className="grid grid-cols-12 gap-4">
                    <div className="col-span-5">
                      <Input placeholder="Description (e.g., Dinner at Cafe)" value={expenseForm.description} onChange={(e) => setExpenseForm({ ...expenseForm, description: e.target.value })} className="border-gray-300 focus:ring-purple-500 focus:border-purple-500" aria-label="Expense description" />
                    </div>
                    <div className="col-span-3">
                      <Input type="number" placeholder="Amount" value={expenseForm.amount} onChange={(e) => setExpenseForm({ ...expenseForm, amount: e.target.value })} className="border-gray-300 focus:ring-purple-500 focus:border-purple-500" aria-label="Expense amount" min="0" step="0.01" />
                    </div>
                    <div className="col-span-3">
                      <Input placeholder="Paid By" value={expenseForm.paidBy} onChange={(e) => setExpenseForm({ ...expenseForm, paidBy: e.target.value })} className="border-gray-300 focus:ring-purple-500 focus:border-purple-500" aria-label="Expense paid by" />
                    </div>
                    <div className="col-span-1">
                      <Button type="submit" className="w-full bg-purple-500 hover:bg-purple-600 text-white rounded-full" aria-label="Add expense">Add</Button>
                    </div>
                  </form>
                </Card>
                {expenses.length === 0 ? (<p className="text-sm text-gray-600">No expenses added yet.</p>) : (expenses.map((expense) => (
                  <Card key={expense.id} className="p-4 mb-4 bg-white shadow-md">
                    <div className="flex items-center justify-between gap-4">
                      <p className="flex-1 text-sm font-medium text-gray-800 truncate">{expense.description}</p>
                      <p className="text-sm text-gray-600 w-24">{expense.paidBy}</p>
              ...
              <TabsContent value="chat">
                <h2 className="text-lg font-semibold text-gray-800 mb-4">Group Chat</h2>
                <div ref={messagesContainerRef} className="h-64 overflow-y-auto border border-gray-300 rounded-lg p-4 mb-4 bg-white">
                  {chatMessages.map((msg) => (
                    <div key={msg.id} className="mb-2">
                      <p className="text-sm font-medium text-gray-800">
                        {msg.userName} <span className="text-xs text-gray-500">{new Date(msg.timestamp).toLocaleTimeString()}</span>
                ...
                }} className="border-gray-300 focus:ring-purple-500 focus:border-purple-500" aria-label="Send chat message" />
              </TabsContent>
            </Tabs>
          </div>
          ...
          <div className="w-full md:w-80 bg-white rounded-lg shadow p-4">
            ...
            {aiSuggestions.map((suggestion, index) => (
              <Card key={index} className="p-4 mb-4 bg-white shadow-md">
                <div className="flex items-start gap-4">
                  <img src={suggestion.image} alt={suggestion.title} className="w-16 h-16 rounded-lg object-cover" />
                  <div>
                    <h4 className="font-medium text-gray-800">{suggestion.title}</h4>
                    <Badge className="my-1 bg-purple-100 text-purple-800">{suggestion.type}</Badge>
                    <p className="text-xs text-gray-600">{suggestion.description}</p>
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (!response.ok) {
                            ...
                            const serverMsg = body?.message || (await response.text()).slice(0, 200) || response.statusText;
                          ...
                          setTripDays((prev) =>
                            prev.map((d) => (d.day === day ? { ...d, activities: (d.activities || 0) + 1 } : d))
                          );
                          toast.success("Activity added to plan!");
                        } catch (err: any) {
